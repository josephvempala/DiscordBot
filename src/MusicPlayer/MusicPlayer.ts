...
export class GuildPlayer {
	...
	private registerGuildPlayerEventListeners() {
		...
		this.player.addListener('error', async (e: any) => {
			if (e.message === 'Status code: 403' && this.currentlyPlaying && this.replayRetries < 5) {
				...
				logger.error(`Error occurred in music player while attempting to play '${this.currentlyPlaying?.title ?? ''}' : ${e.message}`, this.guildId);
			}
			this.player.stop();
			this.playNext();
		});
	...
	private async searchByQueryString(param: string) {
		...
		const urls = await getYoutubeSearchResult(param, 1);
		newMessage.delete();
		return urls;
	}
	...
	getQueue(param?: string) {
		...
		if (!param) {
			this.queue.slice(0, 5).forEach((x, i) => (msg += `**#${i + 2}** ${x.title} \`[${x.isLiveStream ? 'LIVE 🔴' : secondsToTime(x.length)}]\`\n`));
			return msg;
		}
		if (isNaN(+param)) {

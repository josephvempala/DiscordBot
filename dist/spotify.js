...
async function parseSpotifyPlayParameter(param) {
    ...
    if (spotifyAudioInfo) {
        return [{
            url: spotifyAudioInfo.url,
            title: spotifyAudioInfo.title,
            length: spotifyAudioInfo.duration ? spotifyAudioInfo.duration : 0,
            type: IBasicVideoInfo_1.VideoInfoType.Spotify
        }];
    }
    return null;
}

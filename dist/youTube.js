...
async function parseYouTubePlayParameter(param) {
    ...
    try {
        ...
        if (playlistID) {
            ...
            playlistQueryResult.items.map(x => result.push({ url: x.url, title: x.title, length: x.durationSec, type: IBasicVideoInfo_1.VideoInfoType.YouTube }));
            return result;
        }
        const basicVideoInfo = await (0, ytdl_core_discord_1.getBasicInfo)(param).catch(() => null);
        if (basicVideoInfo)
            return [{ url: param, title: basicVideoInfo.player_response.videoDetails.title, length: +basicVideoInfo.videoDetails.lengthSeconds, type: IBasicVideoInfo_1.VideoInfoType.YouTube }];
        ...
        if (finalLinks) {
            ...
            return [{ url: link.url, title: link.title, length: +basicVideoInfo.videoDetails.lengthSeconds, type: IBasicVideoInfo_1.VideoInfoType.YouTube }];
        }
    }
    catch (e) {
        console.log(e);
